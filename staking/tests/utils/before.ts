...
export async function startValidator(portNumber: number, config: AnchorConfig) {
    ...
    // Airdrop some funds and deploy the multisig program to our local cluster
    console.log("Deploying multisig...")
    shell.exec(`solana airdrop 100000`)
    shell.exec(
        `solana program deploy /home/healeyr/Code/multisig-coral/target/verifiable/coral_multisig.so --program-id /home/healeyr/Code/multisig-coral/target/deploy/coral_multisig-keypair.json `
    );
